...
"""
Batched 8-head multi-flow warp-assist UNet.

This version keeps the current multiscale warp-assist architecture but removes
the head-dependent Python loop inside the conditioning path.

Main changes:
- flow hypotheses are predicted in parallel as one tensor
- conditioning features are projected once and chunked into heads
- warping is done by a batched grid_sample over the folded head axis
- head scoring is done by a shared scorer over the folded head axis
- the main UNet remains the same stage layout, with multihead flow-FiLM blocks
...
def batched_grid_warp_head_chunks(features: torch.Tensor, flows: torch.Tensor) -> torch.Tensor:
    """
...
class MultiHeadWarpFiLMBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, spatial: torch.Tensor, flow_heads: torch.Tensor) -> torch.Tensor:
        b, c, h, w = x.shape
        ...
        warped = batched_grid_warp_head_chunks(warp, flow_heads)  # [B, Hh, head_ch, H, W]

        score_in = torch.cat([warped, warp], dim=2).reshape(b * self.num_flow_heads, 2 * self.head_ch, h, w)
...
class PyramidEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.downsamples = nn.ModuleList([nn.AvgPool2d(2) for _ in range(len(channels) - 1)])
    ...
    def __init__(
        ...
        for i in range(num_blocks):
            self.res_blocks.append(
                ResBlock(in_ch if i == 0 else out_ch, out_ch, cond_dim=cond_dim, dropout=dropout)
            )
            self.flow_blocks.append(
                MultiHeadWarpFiLMBlock(out_ch=out_ch, spatial_ch=spatial_ch, num_flow_heads=num_flow_heads)
            )
...
class UpStage(nn.Module):
    def __init__(
        ...
        for i in range(num_blocks):
            self.res_blocks.append(
                ResBlock((in_ch + skip_ch) if i == 0 else out_ch, out_ch, cond_dim=cond_dim, dropout=dropout)
            )
            self.flow_blocks.append(
                MultiHeadWarpFiLMBlock(out_ch=out_ch, spatial_ch=spatial_ch, num_flow_heads=num_flow_heads)
            )
...
class FlowDownStage(nn.Module):
    def __init__(self, in_ch: int, out_ch: int, num_blocks: int, dropout: float = 0.0):
        ...
        self.blocks = nn.ModuleList(
            [
                ResBlock(in_ch if i == 0 else out_ch, out_ch, cond_dim=None, dropout=dropout)
                for i in range(num_blocks)
            ]
        )
        self.downsample = nn.AvgPool2d(2)
...
class FlowUpStage(nn.Module):
    def __init__(self, in_ch: int, skip_ch: int, out_ch: int, num_blocks: int, dropout: float = 0.0):
        super().__init__()
        self.blocks = nn.ModuleList(
            [
                ResBlock((in_ch + skip_ch) if i == 0 else out_ch, out_ch, cond_dim=None, dropout=dropout)
                for i in range(num_blocks)
            ]
        )
        self.upsample = nn.Upsample(scale_factor=2, mode="bilinear", align_corners=False)
...
class OpticalFlowUNet(nn.Module):
    ...
    def __init__(
        ...
        self.mid_blocks = nn.ModuleList([ResBlock(ch1, ch1, cond_dim=None, dropout=dropout) for _ in range(2)])

        self.up1 = FlowUpStage(ch1, ch1, ch0, num_blocks=2, dropout=dropout)
        self.up2 = FlowUpStage(ch0, ch0, ch0, num_blocks=1, dropout=dropout)
        ...
        for layer in (self.flow_mid, self.flow_half, self.flow_full):
            nn.init.zeros_(layer.weight)
            nn.init.zeros_(layer.bias)

    def forward(self, frame_A: torch.Tensor, frame_B: torch.Tensor) -> List[torch.Tensor]:
        # B -> A flow convention
        ...
        flow_coarse = self.flow_mid(x).view(x.shape[0], self.num_flow_heads, 2, x.shape[-2], x.shape[-1])

        x = self.up1(x, skip1)
        flow_half = upsample_flow_heads(flow_coarse, x.shape[-2:]) + self.flow_half(x).view(
            x.shape[0], self.num_flow_heads, 2, x.shape[-2], x.shape[-1]
        )

        x = self.up2(x, skip0)
        flow_full = upsample_flow_heads(flow_half, x.shape[-2:]) + self.flow_full(x).view(
            x.shape[0], self.num_flow_heads, 2, x.shape[-2], x.shape[-1]
        )
...
class MultiHeadWarpAssistUNetBatched8(nn.Module):
    def __init__(
        ...
        self.mid_blocks = nn.ModuleList([ResBlock(ch1, ch1, cond_dim=cond_dim, dropout=dropout) for _ in range(4)])
