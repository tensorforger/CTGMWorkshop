"""
Batched 32-head multi-flow warp-assist UNet.
...
class OpticalFlowUNet(nn.Module):
    ...
    def forward(
        ...
        flow_coarse = self.flow_mid(x).view(
            x.shape[0], self.num_flow_heads, 2, x.shape[-2], x.shape[-1]
        )

        x = self.up1(x, skip1)
        flow_half = upsample_flow_heads(flow_coarse, x.shape[-2:]) + self.flow_half(
            x
        ).view(x.shape[0], self.num_flow_heads, 2, x.shape[-2], x.shape[-1])

        x = self.up2(x, skip0)
        flow_full = upsample_flow_heads(flow_half, x.shape[-2:]) + self.flow_full(
            x
        ).view(x.shape[0], self.num_flow_heads, 2, x.shape[-2], x.shape[-1])
