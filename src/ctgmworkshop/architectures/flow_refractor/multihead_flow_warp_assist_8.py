...
class MultiHeadSpatialFiLMResBlock(nn.Module):
    ...
    def __init__(
        ...
        self.spatial_proj = nn.ModuleList(
            [nn.Conv2d(spatial_ch, 2 * out_ch, 1) for _ in range(num_flow_heads)]
        )
        self.head_score = nn.ModuleList(
            [nn.Conv2d(spatial_ch, 1, 1) for _ in range(num_flow_heads)]
        )
    ...
    def forward(
        ...
        if spatial_heads is None:
            return x
        assert len(spatial_heads) == self.num_flow_heads, (
            len(spatial_heads),
            self.num_flow_heads,
        )
...
class OpticalFlowUNet(nn.Module):
    ...
    pyramid ordered as [full, half, quarter], and each scale contains 16 flow
    hypotheses.
    """
...
class MultiHeadWarpAssistUNetV4(nn.Module):
    """
    Multiscale Warp-Assist UNet with dedicated OpticalFlowUNet and 16 flow hypotheses
    per scale.
    """
...
FlowRefractorModel = MultiHeadWarpAssistUNetV4
