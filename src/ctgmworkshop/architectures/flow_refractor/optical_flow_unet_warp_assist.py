"""
Multiscale Warp-Assist UNet with a dedicated OpticalFlow UNet.

Next experiment idea implemented here:

1. The flow estimator is no longer a lightweight head. It is its own UNet.
2. The flow UNet mirrors the main denoising UNet topology, but uses roughly
   2x fewer residual blocks per stage.
3. Optical flow is produced from the decoder path, so the full-resolution
   decoder output directly supplies the highest-resolution warp used inside
   the main UNet.
4. The edit branch remains a multiscale encoder, and warped edit features are
   injected into the main UNet via spatial FiLM.
...
Pipeline
--------
- OpticalFlowUNet(frame_A, frame_B) -> flow pyramid [full, half, quarter]
- EditPyramidEncoder(frame_A, edited_frame_A) -> edit feature pyramid
- Warp edit features at matching scales
- Inject warped features into the denoiser via spatial FiLM conditioning
"""
...
def warp_features(features: torch.Tensor, flow: torch.Tensor) -> torch.Tensor:
    ...
    The sampling grid is built as base_grid + flow, so this is a target-to-
    source warp (B-space -> A-space), which is the convention needed for the
    edit features.
    """
    ...
    def __init__(
        ...
        for i in range(num_blocks):
            if use_spatial:
                ...
                block: nn.Module = SpatialFiLMResBlock(
...
class OpticalFlowUNet(nn.Module):
    ...
    pyramid ordered as [full, half, quarter].
    """
    ...
    def __init__(
        ...
        self.flow_mid = nn.Conv2d(ch1, 2, 1)
        self.flow_half = nn.Conv2d(ch0, 2, 1)
        self.flow_full = nn.Conv2d(ch0, 2, 1)
    ...
    def forward(
        ...
        flow_coarse = self.flow_mid(x)

        x = self.up1(x, skip1)
        flow_half = upsample_flow(flow_coarse, x.shape[-2:]) + self.flow_half(x)

        x = self.up2(x, skip0)
        flow_full = upsample_flow(flow_half, x.shape[-2:]) + self.flow_full(x)

        return [flow_full, flow_half, flow_coarse]


class MultiScaleWarpAssistUNetV2(nn.Module):
    """
    Multiscale Warp-Assist UNet with dedicated OpticalFlowUNet.
    """
    ...
    def forward(
        ...
        # Warp edit features at each scale with the corresponding flow estimate.
        warped_edit_pyr = [
...
FlowRefractorModel = MultiScaleWarpAssistUNetV2


if __name__ == "__main__":
    # Lightweight smoke test; the training configuration can use larger shapes.
    model = MultiScaleWarpAssistUNetV2(sample_channels=8, base_channels=32)
    b, c, h, w = 1, 8, 16, 16
