"""
Batched 8-head multi-flow warp-assist UNet with a deeper flow backbone.

This version keeps the current multiscale warp-assist architecture and the
batched multi-head conditioning path, but makes the dedicated OpticalFlow UNet
one stage deeper in the bottleneck while keeping the same three output flow
resolutions used by the main denoising UNet.
...
- the flow UNet has an extra internal down/up stage below the quarter scale
