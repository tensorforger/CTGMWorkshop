"""
Merged Flow-Denoising UNet with per-block multi-flow FiLM conditioning.

Experiment idea implemented here:

1. Keep a separate multiscale encoder for frame_A and frame_B.
   The two streams share weights (Siamese encoder) and their features are
   concatenated at each scale for UNet conditioning.
2. Keep a separate multiscale edit encoder for (frame_A, edited_frame_A).
3. Remove all general attention from the main UNet.
4. After every residual block in the UNet, insert a structural flow-guided
   mixture block:
   - predict multiple optical flow hypotheses
   - project edit features into a warping latent space
   - split the latent into per-flow chunks
   - warp each chunk separately
   - score each chunk per pixel using both warped and unwarped features
   - softmax over flow hypotheses
   - fuse the warped chunks
   - project back to 2 * feature_channels
   - apply as FiLM to the UNet feature map
5. Use the same block counts as the baseline UNet:
   down: 2 -> 4
   mid: 8
   up: 4 -> 2
...
Flow convention
---------------
Flows are B->A sampling offsets, so they can warp edit features from A-space
into the B-space used by the denoising UNet.
"""
...
def upsample_flow_heads(flow: torch.Tensor, target_hw: Tuple[int, int]) -> torch.Tensor:
    """
    Upsample [B, Hh, 2, H, W] flow and rescale offsets to the new pixel grid.
    """
...
def warp_features_multihead(
    ...
    return [warp_features(features, flows[:, i]) for i in range(flows.shape[1])]
...
class SiamesePyramidEncoder(nn.Module):
    ...
    Shared weights can be used for frame_A and frame_B.
    """
    ...
    def __init__(
        ...
        self.downsamples = nn.ModuleList(
            [nn.AvgPool2d(2) for _ in range(len(self.channels) - 1)]
        )
...
class FlowGuidedFiLMBlock(nn.Module):
    """
    Structural block inserted after every UNet residual block.

    It:
      1. predicts multiple flow hypotheses from the frame-context features and
         the current UNet state,
      2. projects edit features into a warping latent space,
      3. chunks that latent into one chunk per flow hypothesis,
      4. warps each chunk separately,
      5. scores each head per pixel using warped + unwarped features,
      6. softmax-fuses the heads,
      7. projects back to 2 * feature channels,
      8. applies FiLM to the UNet feature map.
    """
    ...
    def __init__(
        ...
        warp_ch = _round_up_multiple(max(feat_ch, num_flow_heads * 16), num_flow_heads)
        self.warp_ch = warp_ch
        self.chunk_ch = warp_ch // num_flow_heads

        flow_hidden = max(32, feat_ch // 2)
        score_hidden = max(32, self.chunk_ch)

        self.flow_net = nn.Sequential(
            conv_norm_act(feat_ch + frame_ctx_ch, flow_hidden),
            conv_norm_act(flow_hidden, flow_hidden),
            nn.Conv2d(flow_hidden, 2 * num_flow_heads, 1),
        )

        self.edit_to_warp = nn.Conv2d(edit_ctx_ch, warp_ch, 1)
        self.frame_to_score = nn.Conv2d(frame_ctx_ch, self.chunk_ch, 1)
    ...
    def forward(
        ...
        edit_ctx: torch.Tensor,
    ) -> torch.Tensor:
        if frame_ctx.shape[-2:] != x.shape[-2:]:
            frame_ctx = F.interpolate(
                frame_ctx, size=x.shape[-2:], mode="bilinear", align_corners=False
            )
        if edit_ctx.shape[-2:] != x.shape[-2:]:
            edit_ctx = F.interpolate(
                edit_ctx, size=x.shape[-2:], mode="bilinear", align_corners=False
            )

        flow = _reshape_flow_heads(
            self.flow_net(torch.cat([x, frame_ctx], dim=1)), self.num_flow_heads
        )

        edit_latent = self.edit_to_warp(edit_ctx)
        latent_chunks = list(torch.chunk(edit_latent, self.num_flow_heads, dim=1))
        score_ctx = self.frame_to_score(frame_ctx)

        warped_chunks = []
        score_logits = []

        for i in range(self.num_flow_heads):
            warped_i = warp_features(latent_chunks[i], flow[:, i])
            warped_chunks.append(warped_i)
            score_in = torch.cat([warped_i, latent_chunks[i], score_ctx], dim=1)
            score_logits.append(self.head_scores[i](score_in))
...
class ContextualResidualFlowBlock(nn.Module):
    """
    Residual block followed by FlowGuidedFiLMBlock.
    The UNet receives concatenated frame-A/frame-B features at every scale.
    """
    ...
    def __init__(
        ...
        super().__init__()
        self.res = ResBlock(
            in_ch + frame_ctx_ch, out_ch, cond_dim=cond_dim, dropout=dropout
        )
        self.flow_film = FlowGuidedFiLMBlock(
            feat_ch=out_ch,
            frame_ctx_ch=frame_ctx_ch,
            edit_ctx_ch=edit_ctx_ch,
    ...
    def forward(
        ...
        x = self.res(torch.cat([x, frame_ctx], dim=1), cond)
        x = self.flow_film(x, frame_ctx, edit_ctx)
        return x
...
class DownStage(nn.Module):
    ...
    def forward(
        ...
        for block in self.blocks:
            x = block(x, cond, frame_ctx, edit_ctx)
        skip = x
        x = self.downsample(x)
        return x, skip
...
class MergeFlowDenoiseUNet(nn.Module):
    ...
    Output:
      - predicted residual / denoised latent
    """
    ...
    def __init__(
        ...
        assert len(pyramid_channels) == 3, (
            "Expected 3 pyramid scales: full, half, quarter."
        )

        self.conditioning = TimeCondEncoder(time_dim=time_dim, cond_dim=cond_dim)

        # Shared encoder for frame_A and frame_B.
        ...
        # Separate edit encoder.
        self.edit_encoder = SiamesePyramidEncoder(
        ...
        edit_ctx_ch = list(pyramid_channels)
    ...
    def forward(
        ...
        frame_ctx_pyr = [
            torch.cat([a, b], dim=1) for a, b in zip(frame_A_pyr, frame_B_pyr)
        ]
        ...
        x, skip0 = self.down1(x, cond, frame_ctx_pyr[0], edit_pyr[0])
        x, skip1 = self.down2(x, cond, frame_ctx_pyr[1], edit_pyr[1])

        for block in self.mid_blocks:
            x = block(x, cond, frame_ctx_pyr[2], edit_pyr[2])

        x = self.up1(x, skip1, cond, frame_ctx_pyr[1], edit_pyr[1])
        x = self.up2(x, skip0, cond, frame_ctx_pyr[0], edit_pyr[0])
